...
export async function openBuilderMode(
  ...
  arg: { initialContent?: string | string[]; initialContentSource?: ImportSource } | undefined
) {
  logger.log('Starting BuilderMode - preparing environment...');

  BuilderModeProvider.show(context, arg?.initialContent, arg?.initialContentSource);
}
