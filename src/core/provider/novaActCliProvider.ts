...
export class NovaActCliProvider {
  ...
  // user-controllable via the novaAct.cliPath setting, so it must not reach a
  // shell command string (CWE-78).
  ...
  async executeNovaActCommand(args: string[], webview?: vscode.Webview): Promise<string> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // region / dir / role-arn are NEVER interpreted by a shell (CWE-78).
  ...
  async handleInvokeRuntime(
    ...
    try {
      ...
      // never parsed by a shell (CWE-78).
      const runArgs = [
        'workflow',
        'run',
        '--name',
