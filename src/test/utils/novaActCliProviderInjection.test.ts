...
// Regression: OS command injection in the Nova Act CLI provider (Vigilance #3872004 / CWE-78).
// Workflow name / payload / region flow from an untrusted webview message into a command
// STRING that is run with `shell: true`. Shell metacharacters in those fields ($(...), ;, ',
// backticks) are therefore interpreted by /bin/sh, yielding RCE. The fix passes the CLI path
